# -*- coding: utf-8 -*-
"""
Script DEFINITIVO para deletar TODAS as transferências de Vacas Descarte sem saldo
Verifica TODOS os planejamentos e deleta transferências sem saldo
"""
import os
import sys
import django
...
def deletar_todas_sem_saldo():
    """Deleta TODAS as transferências sem saldo de TODOS os planejamentos"""
    
    print("=" * 80)
    print("DELETAR TODAS TRANSFERENCIAS SEM SALDO - DEFINITIVO")
    print("=" * 80)
    ...
    # Verificar estado final no planejamento mais recente
    ...
    if transferencias_restantes.exists():
        ...
        for t in transferencias_restantes:
            saldo_antes = calcular_saldo_disponivel(
                canta_galo, 
                categoria_descarte, 
                t.data_movimentacao - timedelta(days=1),
                planejamento_atual
            )
            print(f"    {t.data_movimentacao.strftime('%d/%m/%Y')}: {t.quantidade} vacas (saldo antes: {saldo_antes})")
