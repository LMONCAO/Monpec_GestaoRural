...
# Primeiro tenta com settings padrão (desenvolvimento)
python manage.py migrate --noinput
if ($LASTEXITCODE -ne 0) {
    Write-Host "Tentando com settings de produção..." -ForegroundColor Yellow
    python manage.py migrate --settings=sistema_rural.settings_producao --noinput
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Migrações aplicadas (produção)" -ForegroundColor Green
    } else {
...
# Tenta primeiro com settings padrão
python manage.py collectstatic --noinput --clear
if ($LASTEXITCODE -ne 0) {
    Write-Host "Tentando com settings de produção..." -ForegroundColor Yellow
    python manage.py collectstatic --settings=sistema_rural.settings_producao --noinput --clear
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Arquivos estáticos coletados (produção)" -ForegroundColor Green
    } else {
