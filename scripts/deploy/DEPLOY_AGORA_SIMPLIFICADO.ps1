...
# Verificar gcloud
try { gcloud --version | Out-Null } catch { Write-Host "❌ gcloud não encontrado!" -ForegroundColor Red; exit 1 }
...
if (Test-Path ".env") {
    ...
    if ($mercadopagoToken) {
        Write-Host "✅ Credenciais Mercado Pago encontradas" -ForegroundColor Green
    }
}
...
# Usar Dockerfile.prod para produção
gcloud builds submit --tag $imageTag --file Dockerfile.prod --quiet
...
Write-Host ""
Write-Host "⚠️  Execute criar_admin_producao.py para configurar o admin" -ForegroundColor Yellow
Write-Host ""
