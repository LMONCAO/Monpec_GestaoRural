...
# ========================================
# Execute este script para excluir arquivos identificados como desnecessários
# IMPORTANTE: Faça backup antes de executar!

Write-Host "🧹 LIMPEZA DE ARQUIVOS DESNECESSÁRIOS" -ForegroundColor Green
Write-Host "=====================================" -ForegroundColor Yellow
Write-Host ""
Write-Host "⚠️  ATENÇÃO: Este script irá EXCLUIR arquivos!" -ForegroundColor Red
...
# Função para excluir com segurança
function Excluir-Arquivo {
    param($caminho, $tipo)
    if (Test-Path $caminho) {
        try {
            Remove-Item -Path $caminho -Recurse -Force -ErrorAction Stop
            Write-Host "✅ Excluído: $tipo - $caminho" -ForegroundColor Green
            $script:excluidos++
        } catch {
            Write-Host "❌ Erro ao excluir $caminho : $_" -ForegroundColor Red
            $script:erros++
        }
    } else {
...
# 3. EXCLUIR SCRIPTS TEMPORÁRIOS PYTHON
Write-Host ""
Write-Host "🐍 Excluindo scripts Python temporários..." -ForegroundColor Cyan
$scriptsPython = @(
...
foreach ($script in $scriptsPython) {
    Excluir-Arquivo $script "Script temporário"
}

# 4. EXCLUIR SCRIPTS DE INSTALAÇÃO ANTIGOS
Write-Host ""
Write-Host "🔧 Excluindo scripts de instalação antigos..." -ForegroundColor Cyan
$scriptsInstalacao = @(
...
foreach ($script in $scriptsInstalacao) {
    Excluir-Arquivo $script "Script de instalação"
}
...
Write-Host ""
Write-Host "📝 Excluindo outros scripts temporários..." -ForegroundColor Cyan
$outrosScripts = @(
...
foreach ($script in $outrosScripts) {
    Excluir-Arquivo $script "Script/arquivo temporário"
}
...
Write-Host "=====================================" -ForegroundColor Yellow
Write-Host "✅ Arquivos/Pastas excluídos: $excluidos" -ForegroundColor Green
