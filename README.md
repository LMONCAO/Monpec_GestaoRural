...
- PostgreSQL (recomendado) ou SQLite3 (padrão)

## 🐘 Configuração PostgreSQL (Recomendado)

Para usar PostgreSQL em desenvolvimento:

1. Instale PostgreSQL no Windows
2. Execute o script de configuração:
   ```batch
   .\scripts\instalar_e_configurar_postgresql.ps1
   ```
3. O arquivo `.env.local` será criado automaticamente com as configurações

**Benefícios do PostgreSQL:**
- Compatibilidade total com produção (Google Cloud SQL)
- Melhor performance
- Suporte completo aos recursos do Django
...
2. Configure o PostgreSQL (opcional, mas recomendado):
   ```batch
   .\scripts\instalar_e_configurar_postgresql.ps1
   ```
