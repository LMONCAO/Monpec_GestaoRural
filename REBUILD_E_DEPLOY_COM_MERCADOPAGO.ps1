# Rebuild e Deploy com correção do mercadopago_customer_id
...
Write-Host "🔍 Passo 1: Verificando e criando coluna mercadopago_customer_id..." -ForegroundColor Yellow
...
Write-Host "⏱️  Executando verificação..." -ForegroundColor Yellow
gcloud run jobs execute verificar-mercadopago --region=$REGION --wait

Write-Host ""
Write-Host "📋 Logs da verificação:" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "🔨 Passo 2: Fazendo rebuild da imagem Docker..." -ForegroundColor Yellow
gcloud builds submit --tag $IMAGE_NAME --timeout=20m

Write-Host ""
Write-Host "🚀 Passo 3: Fazendo deploy no Cloud Run..." -ForegroundColor Yellow
