...
(function() {
  ...
  // ========== GERADORES ==========
  const Gerador = {
    // Usar dados reais quando disponíveis, senão gerar
    obterBrinco: () => {
      if (Estado.dadosReais.brincosEstoque.length > 0) {
        const brinco = Estado.dadosReais.brincosEstoque[
          Math.floor(Math.random() * Estado.dadosReais.brincosEstoque.length)
        ];
        return {
          sisbov: brinco.codigo_sisbov || brinco.numero_brinco,
          brinco: brinco.numero_manejo || brinco.numero_brinco.substring(8, 14),
          tipo: 'estoque'
        };
      }
      ...
      return {
        sisbov: sisbov,
        brinco: sisbov.substring(8, 14),
        tipo: 'gerado'
      };
    },
    
    obterAnimal: () => {
      if (Estado.dadosReais.animaisCadastrados.length > 0) {
        const animal = Estado.dadosReais.animaisCadastrados[
          Math.floor(Math.random() * Estado.dadosReais.animaisCadastrados.length)
        ];
        return {
          ...animal,
          tipo: 'cadastrado'
  ...
  const DadosReais = {
    carregar: async function(propriedadeId) {
      if (Estado.dadosReais.carregado) {
        Logs.adicionar('✅ Dados reais já carregados', 'success');
        return;
      }
      
      try {
        Logs.adicionar('📡 Carregando dados reais do sistema...', 'info');
        const url = `/propriedade/${propriedadeId}/curral/api/dados-simulacao/`;
        ...
        if (data.status === 'sucesso') {
          Estado.dadosReais.brincosEstoque = data.brincos_estoque || [];
          Estado.dadosReais.animaisCadastrados = data.animais_cadastrados || [];
          Estado.dadosReais.carregado = true;
          
          // Atualizar total de registros disponíveis
          const totalDisponivel = Estado.dadosReais.brincosEstoque.length + Estado.dadosReais.animaisCadastrados.length;
          if (totalDisponivel > 0) {
            CONFIG.total = Math.min(totalDisponivel, CONFIG.total);
            Logs.adicionar(`✅ Dados carregados: ${data.total_brincos} brincos em estoque, ${data.total_animais} animais cadastrados`, 'success');
            Logs.adicionar(`📊 Total de registros disponíveis: ${totalDisponivel}`, 'info');
          } else {
            Logs.adicionar('⚠️ Nenhum dado real encontrado, usando dados simulados', 'warning');
          }
        } else {
          throw new Error(data.mensagem || 'Erro ao carregar dados');
        }
      } catch (error) {
        Logs.adicionar(`⚠️ Erro ao carregar dados reais: ${error.message}. Usando dados simulados.`, 'warning');
        Estado.dadosReais.carregado = false;
      }
    }
  };
  ...
  const Processador = {
    processar: async function() {
      ...
      try {
        ...
        let dadosAnimal;
        let usarDadosReais = Math.random() < 0.7 && Estado.dadosReais.carregado; // 70% chance de usar dados reais
        
        if (usarDadosReais && Estado.dadosReais.animaisCadastrados.length > 0) {
          // Usar animal cadastrado real
          dadosAnimal = Gerador.obterAnimal();
          Logs.adicionar(`📋 Usando animal cadastrado real: ${dadosAnimal.numero_brinco || dadosAnimal.codigo_sisbov}`, 'info');
        } else if (usarDadosReais && Estado.dadosReais.brincosEstoque.length > 0) {
          // Usar brinco do estoque
          dadosAnimal = Gerador.obterBrinco();
          Logs.adicionar(`📦 Usando brinco do estoque: ${dadosAnimal.brinco}`, 'info');
        } else {
          ...
          Logs.adicionar(`🎲 Gerando dados aleatórios`, 'info');
        }
        
        const sisbov = dadosAnimal.codigo_sisbov || dadosAnimal.sisbov || Gerador.sisbov();
        const brinco = dadosAnimal.numero_manejo || dadosAnimal.brinco || sisbov.substring(8, 14);
        const raca = dadosAnimal.raca || Gerador.raca();
        const sexo = dadosAnimal.sexo || Gerador.sexo();
        const idade = dadosAnimal.idade_meses || dadosAnimal.idade || Gerador.idade();
        
        // Calcular peso baseado em dados reais ou gerar
        let peso;
        if (dadosAnimal.peso_atual) {
          // Usar peso real com pequena variação
          const pesoBase = parseFloat(dadosAnimal.peso_atual);
          const variacao = (Math.random() * 10 - 5); // ±5kg
          peso = (pesoBase + variacao).toFixed(1).replace('.', ',');
        } else {
          peso = Gerador.peso();
        }
    ...
    cadastrarEstoque: async function(brinco, sisbov, raca, sexo, idade) {
      ...
      if (campoIdade) {
        ...
        if (typeof calcularDataNascimentoPorIdadeMeses === 'function') {
          try {
            const dataCalc = calcularDataNascimentoPorIdadeMeses(idade);
  ...
  const Controle = {
    iniciar: async function() {
      ...
      // Carregar dados reais antes de iniciar
      const propId = window.location.pathname.match(/propriedade\/(\d+)/);
      if (propId && !Estado.dadosReais.carregado) {
        await DadosReais.carregar(propId[1]);
      }
      ...
      const totalDisponivel = Estado.dadosReais.brincosEstoque.length + Estado.dadosReais.animaisCadastrados.length;
      const usandoDadosReais = totalDisponivel > 0;
      
      Logs.adicionar(`🚀 Simulação iniciada - Processando ${CONFIG.total} registros...`, 'info');
      if (usandoDadosReais) {
        Logs.adicionar(`✅ Usando dados reais: ${Estado.dadosReais.brincosEstoque.length} brincos + ${Estado.dadosReais.animaisCadastrados.length} animais`, 'success');
      } else {
        Logs.adicionar(`⚠️ Usando dados simulados (nenhum dado real disponível)`, 'warning');
      }
  ...
  document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Carregar dados reais ao inicializar (em background)
    const propId = window.location.pathname.match(/propriedade\/(\d+)/);
    if (propId) {
      // Carregar dados em background sem bloquear a interface
      DadosReais.carregar(propId[1]).catch(err => {
        console.warn('Erro ao carregar dados reais:', err);
      });
    }
